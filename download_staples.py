...
def manual_login_and_save():
    ...
    with sync_playwright() as p:
        ...
        # Only prompt for login if session is expired (redirected to login page)
...
def fetch_order_page(session, page_number, order_type="in-store"):
    ...
    if response.status_code != 200:
        print(
            f"Failed to fetch {order_type} page {page_number}. Status: {response.status_code}"
        )
        return None
    try:
        ...
        while current_page <= total_pages:
            try:
                list_data = fetch_order_page(
                    session, current_page, order_type=order_type
                )
                ...
                for order in orders:
                    ...
                    if cache_file.exists():
                        with open(cache_file, "r") as f:
                            cached = json.load(f)
                        cached["_order_type"] = cached.get("_order_type", order_type)
